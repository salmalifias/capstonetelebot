...
def send_plot(message):
    ...
    if selected_campaign_id in unique_campaign:
        ...
        # get age categories
        df_plot_age = df_plot.reset_index()
        df_plot_age = df_plot_age['age']
        ...
        axes[0].bar(x=df_plot_age, height=df_plot['spent'], color='#AE2024')
        ...
        # GET DATA MAX MIN AGE
        df_plot2 = df_plot.reset_index()
        max_index = df_plot2[['spent','approved_conversion','cpc']].idxmax()
        max_age = df_plot2.loc[max_index, 'age']
        min_index = df_plot2[['spent','approved_conversion','cpc']].idxmin()
        min_age = df_plot2.loc[min_index, 'age']
...
def echo_all(message):
    # TO DO: emoji
    emot = emoji.emojize(':slightly_frowning_face:')
    with open('template_text/default.txt', mode='r', encoding='utf-8') as f:
        temp = Template(f.read())
        default = temp.substitute(EMOJI = emot)
